...
const SideNavigation = ({ navLinks, activeLink, setActiveLink }: SideNavigationProps) => {
  ...
  return (
    <motion.div 
      className="fixed right-4 md:right-6 lg:right-10 top-1/2 transform -translate-y-1/2 z-50"
      ...
      >
        ...
        {navLinks.map((link, index) => (
          ...
          >
            ...
            {/* Resaltado de icono activo mejorado */}
            <AnimatePresence>
              {activeLink === link.name && (
                <motion.div 
                  className="absolute inset-0 rounded-full border-2 border-[#9f7aea]/70"
            ...
            {/* Animación de pulso en hover mejorada */}
            <AnimatePresence>
              {hoveredIcon === link.name && hoveredIcon !== activeLink && (
                <motion.div
                  ...
                  animate={{ 
                    opacity: [0, 0.3, 0], 
                    scale: [0.8, 1.5, 2],
                    backgroundColor: ["rgba(159, 122, 234, 0.2)", "rgba(159, 122, 234, 0.1)", "rgba(159, 122, 234, 0)"]
                  }}
